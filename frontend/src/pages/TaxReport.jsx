...
const API_URL = 'http://localhost:5000/api/tax-report';

const TaxReport = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ year: '', branch_id: '', department_id: '', employee_id: '', include_ssf: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ year: '', branch_id: '', department_id: '', employee_id: '', include_ssf: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ year: item.year || '', branch_id: item.branch_id || '', department_id: item.department_id || '', employee_id: item.employee_id || '', include_ssf: item.include_ssf || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Tax Report Management</h4>
