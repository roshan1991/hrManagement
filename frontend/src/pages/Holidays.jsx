...
const Holidays = () => {
  ...
  const [formData, setFormData] = useState({ event: '', event_date: '', is_public_holiday: '', note: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ event: '', event_date: '', is_public_holiday: '', note: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ event: item.event || '', event_date: item.event_date || '', is_public_holiday: item.is_public_holiday || '', note: item.note || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Holidays Management</h4>
      ...
      <Modal show={show} onHide={handleClose} size="lg">
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Is Public Holiday?</Form.Label>
              <Form.Control 
                type="text"
                value={formData.is_public_holiday} 
                onChange={e => setFormData({...formData, is_public_holiday: e.target.value})} 
              />
            </Form.Group>
            <Form.Group className="mb-3">
