...
const API_URL = 'http://localhost:5000/api/location-logs';

const LocationLogs = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', department_id: '', employee_id: '', date: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', department_id: '', employee_id: '', date: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', department_id: item.department_id || '', employee_id: item.employee_id || '', date: item.date || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Location Logs Management</h4>
