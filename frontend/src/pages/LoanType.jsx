...
const API_URL = 'http://localhost:5000/api/loan-type';

const LoanType = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', type: '', name: '', minimum_amount: '', maximum_amount: '', interest_rate: '', interest_type: '', term: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', type: '', name: '', minimum_amount: '', maximum_amount: '', interest_rate: '', interest_type: '', term: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', type: item.type || '', name: item.name || '', minimum_amount: item.minimum_amount || '', maximum_amount: item.maximum_amount || '', interest_rate: item.interest_rate || '', interest_type: item.interest_type || '', term: item.term || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Loan Type Management</h4>
      ...
      <Modal show={show} onHide={handleClose} size="lg">
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Installment Period</Form.Label>
              <Form.Control 
                type="number"
                value={formData.term} 
