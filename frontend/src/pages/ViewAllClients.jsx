...
const API_URL = 'http://localhost:5000/api/view-all-clients';

const ViewAllClients = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', name: '', email: '', contact_no: '', address: '', country: '', avatar: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', name: '', email: '', contact_no: '', address: '', country: '', avatar: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', name: item.name || '', email: item.email || '', contact_no: item.contact_no || '', address: item.address || '', country: item.country || '', avatar: item.avatar || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">View All Clients Management</h4>
