...
const API_URL = 'http://localhost:5000/api/loan-repayment';

const LoanRepayment = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', type_id: '', department_id: '', employee_id: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', type_id: '', department_id: '', employee_id: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', type_id: item.type_id || '', department_id: item.department_id || '', employee_id: item.employee_id || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Loan Repayment Management</h4>
