...
const API_URL = 'http://localhost:5000/api/biometric-device';

const BiometricDevice = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', name: '', serial_number: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', name: '', serial_number: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', name: item.name || '', serial_number: item.serial_number || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Biometric Device Management</h4>
