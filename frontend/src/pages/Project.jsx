...
const SAMPLE = [
  { id: 1, title: 'HR System Revamp', description: 'Overhaul the HR management system', client: 'Margaret Robbins', assigned_to: 'Rajesh Sharma, Priya Thapa', start_date: '2025-05-01', deadline: '2025-08-31', priority: 'High', status: 'In Progress' },
  { id: 2, title: 'Marketing Website', description: 'Build new marketing website', client: 'Contractor1 LLC', assigned_to: 'Amit Poudel', start_date: '2025-06-01', deadline: '2025-07-15', priority: 'Medium', status: 'To Do' },
  { id: 3, title: 'Mobile App v2', description: 'Version 2.0 of mobile application', client: 'Digital Media Group', assigned_to: 'Sunita Gurung, Bikash Rai', start_date: '2025-04-01', deadline: '2025-09-30', priority: 'High', status: 'Done' },
];
...
const emptyForm = { title: '', description: '', client: '', assigned_to: '', start_date: '', deadline: '', priority: 'Medium', status: 'To Do' };
    ...
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0 small">Track and manage all projects</p>
        </div>
        <Button variant="primary" onClick={() => setShow(true)}><i className="ti ti-folder-plus me-1"></i>Create Project</Button>
      </div>
      ...
      <Row className="mb-3 g-2">
        <Col md={5}>
          <InputGroup>
            ...
            <Form.Control placeholder="Search projects..." value={search} onChange={e => setSearch(e.target.value)} />
          </InputGroup>
        </Col>
        <Col md={3}>
      ...
      <Table responsive hover className="align-middle">
        <thead className="table-light">
          <tr><th>#</th><th>Title</th><th>Client</th><th>Team</th><th>Start</th><th>Deadline</th><th>Priority</th><th>Status</th><th className="text-end">Actions</th></tr>
        </thead>
        <tbody>
          {filtered.map((item, idx) => (
            <tr key={item.id}>
              ...
              <td>{item.client || '-'}</td>
              <td><small className="text-muted">{item.assigned_to || '-'}</small></td>
          ...
          {filtered.length === 0 && <tr><td colSpan="9" className="text-center py-4 text-muted">No projects found.</td></tr>}
        </tbody>
      </Table>

      <Modal show={show} onHide={handleClose} size="lg">
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title>{editingId ? 'Edit Project' : 'Create Project'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            <Row className="g-3">
              <Col md={12}><Form.Group><Form.Label>Project Title <span className="text-danger">*</span></Form.Label><Form.Control required value={formData.title} onChange={set('title')} /></Form.Group></Col>
              ...
              <Col md={6}><Form.Group><Form.Label>Assigned To</Form.Label><Form.Select value={formData.assigned_to} onChange={set('assigned_to')}><option value="">Select Team Members</option>{employees.map(e => <option key={e}>{e}</option>)}</Form.Select></Form.Group></Col>
