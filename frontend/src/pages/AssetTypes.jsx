...
const API_URL = 'http://localhost:5000/api/asset-types';

const AssetTypes = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', type: '', name: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', type: '', name: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', type: item.type || '', name: item.name || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Asset Types Management</h4>
