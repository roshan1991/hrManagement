...
const API_URL = 'http://localhost:5000/api/training-type';

const TrainingType = () => {
  const [items, setItems] = useState([]);
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState({ branch_id: '', type: '', title: '' });
  ...
  const handleClose = () => {
    setShow(false);
    setFormData({ branch_id: '', type: '', title: '' });
    setEditingId(null);
  };
  ...
  const handleEdit = (item) => {
    setFormData({ branch_id: item.branch_id || '', type: item.type || '', title: item.title || '' });
  ...
  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Training Type Management</h4>
