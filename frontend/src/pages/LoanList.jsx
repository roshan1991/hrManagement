import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { Button, Form, Modal, Badge, Row, Col, Table } from 'react-bootstrap';

const SAMPLE = [
  { id: 1, employee: 'Rajesh Sharma', loan_type: 'Personal Loan', amount: 150000, interest_rate: 12, duration_months: 24, emi_amount: 7054, reason: 'Medical emergency', approval_status: 'Approved', applied_date: '2025-05-01' },
  { id: 2, employee: 'Priya Thapa', loan_type: 'Home Loan', amount: 500000, interest_rate: 8, duration_months: 60, emi_amount: 10138, reason: 'Home renovation', approval_status: 'Pending', applied_date: '2025-06-01' },
  { id: 3, employee: 'Amit Poudel', loan_type: 'Education Loan', amount: 200000, interest_rate: 10, duration_months: 36, emi_amount: 6453, reason: 'Higher education', approval_status: 'Rejected', applied_date: '2025-04-15' },
];

const LOAN_TYPES = ['Personal Loan', 'Home Loan', 'Education Loan', 'Vehicle Loan', 'Emergency Loan'];
const EMPLOYEES = ['Rajesh Sharma', 'Priya Thapa', 'Amit Poudel', 'Sunita Gurung', 'Bikash Rai'];
const statusColor = { Pending: 'warning', Approved: 'success', Rejected: 'danger' };

const emptyForm = { employee: '', loan_type: '', amount: '', interest_rate: '', duration_months: '', emi_amount: '', reason: '', approval_status: 'Pending', applied_date: '' };

const API_URL = 'http://localhost:5000/api/loan-list';

const LoanList = () => {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetchItems();
  }, []);

  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await axios.get(API_URL);
      setItems(res.data);
      setError(null);
    } catch (err) {
      console.error(err);
      setError('Failed to fetch records.');
    } finally {
      setLoading(false);
    }
  };
  const [show, setShow] = useState(false);
  const [formData, setFormData] = useState(emptyForm);
  const [editingId, setEditingId] = useState(null);

  const handleClose = () => { setShow(false); setFormData(emptyForm); setEditingId(null); };
  const set = (field) => (e) => setFormData({ ...formData, [field]: e.target.value });

  const calcEMI = () => {
    const p = parseFloat(formData.amount) || 0;
    const r = (parseFloat(formData.interest_rate) || 0) / 100 / 12;
    const n = parseInt(formData.duration_months) || 1;
    if (p && r && n) {
      const emi = (p * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
      setFormData(f => ({ ...f, emi_amount: Math.round(emi) }));
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingId) {
        await axios.put(`${API_URL}/${editingId}`, formData);
      } else {
        await axios.post(API_URL, formData);
      }
      fetchItems();
      handleClose();
    } catch (err) {
      console.error(err);
      alert('Failed to save record.');
    }
  };

  const handleEdit = (item) => { setFormData({ ...item }); setEditingId(item.id); setShow(true); };
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await axios.delete(`${API_URL}/${id}`);
        fetchItems();
      } catch (err) {
        console.error(err);
        alert('Failed to delete record.');
      }
    }
  };
  const updateStatus = (id, status) => setItems(items.map(i => i.id === id ? { ...i, approval_status: status } : i));

  return (
    <div className="premium-card p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 className="fw-bold mb-1">Loan Management</h4>
          <p className="text-muted mb-0 small">Manage employee loan applications and approvals</p>
        </div>
        <Button variant="primary" onClick={() => setShow(true)}><i className="ti ti-plus me-1"></i>Add Loan</Button>
      </div>

      <Table responsive hover className="align-middle">
        <thead className="table-light">
          <tr><th>#</th><th>Employee</th><th>Loan Type</th><th>Amount (Rs.)</th><th>Interest</th><th>Duration</th><th>EMI (Rs.)</th><th>Applied</th><th>Status</th><th className="text-end">Actions</th></tr>
        </thead>
        <tbody>
          {items.map((item, idx) => (
            <tr key={item.id}>
              <td>{idx + 1}</td>
              <td className="fw-medium">{item.employee}</td>
              <td>{item.loan_type}</td>
              <td>{Number(item.amount).toLocaleString()}</td>
              <td>{item.interest_rate}%</td>
              <td>{item.duration_months} mo</td>
              <td className="fw-medium">{Number(item.emi_amount).toLocaleString()}</td>
              <td><small>{item.applied_date || '-'}</small></td>
              <td>
                <Badge bg={statusColor[item.approval_status]} className={`bg-label-${statusColor[item.approval_status]} text-${statusColor[item.approval_status]}`}>
                  {item.approval_status}
                </Badge>
              </td>
              <td className="text-end">
                {item.approval_status === 'Pending' && (
                  <>
                    <Button variant="light" size="sm" className="me-1 text-success" onClick={() => updateStatus(item.id, 'Approved')} title="Approve"><i className="ti ti-check"></i></Button>
                    <Button variant="light" size="sm" className="me-1 text-danger" onClick={() => updateStatus(item.id, 'Rejected')} title="Reject"><i className="ti ti-x"></i></Button>
                  </>
                )}
                <Button variant="light" size="sm" className="me-1 text-primary" onClick={() => handleEdit(item)} title="Edit"><i className="ti ti-edit"></i></Button>
                <Button variant="light" size="sm" className="text-danger" onClick={() => handleDelete(item.id)} title="Delete"><i className="ti ti-trash"></i></Button>
              </td>
            </tr>
          ))}
          {items.length === 0 && <tr><td colSpan="10" className="text-center py-4 text-muted">No loan records found.</td></tr>}
        </tbody>
      </Table>

      <Modal show={show} onHide={handleClose} size="lg">
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title>{editingId ? 'Edit Loan' : 'Add Loan Application'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            <Row className="g-3">
              <Col md={6}><Form.Group><Form.Label>Employee <span className="text-danger">*</span></Form.Label><Form.Select required value={formData.employee} onChange={set('employee')}><option value="">Select Employee</option>{EMPLOYEES.map(e => <option key={e}>{e}</option>)}</Form.Select></Form.Group></Col>
              <Col md={6}><Form.Group><Form.Label>Loan Type <span className="text-danger">*</span></Form.Label><Form.Select required value={formData.loan_type} onChange={set('loan_type')}><option value="">Select Loan Type</option>{LOAN_TYPES.map(l => <option key={l}>{l}</option>)}</Form.Select></Form.Group></Col>
              <Col md={4}><Form.Group><Form.Label>Amount (Rs.) <span className="text-danger">*</span></Form.Label><Form.Control type="number" required value={formData.amount} onChange={set('amount')} onBlur={calcEMI} /></Form.Group></Col>
              <Col md={4}><Form.Group><Form.Label>Interest Rate (%)</Form.Label><Form.Control type="number" step="0.1" value={formData.interest_rate} onChange={set('interest_rate')} onBlur={calcEMI} /></Form.Group></Col>
              <Col md={4}><Form.Group><Form.Label>Duration (Months)</Form.Label><Form.Control type="number" value={formData.duration_months} onChange={set('duration_months')} onBlur={calcEMI} /></Form.Group></Col>
              <Col md={6}><Form.Group><Form.Label>EMI Amount (Rs.)</Form.Label><Form.Control type="number" value={formData.emi_amount} onChange={set('emi_amount')} readOnly className="bg-light" /></Form.Group></Col>
              <Col md={6}><Form.Group><Form.Label>Applied Date</Form.Label><Form.Control type="date" value={formData.applied_date} onChange={set('applied_date')} /></Form.Group></Col>
              <Col md={6}><Form.Group><Form.Label>Approval Status</Form.Label><Form.Select value={formData.approval_status} onChange={set('approval_status')}><option>Pending</option><option>Approved</option><option>Rejected</option></Form.Select></Form.Group></Col>
              <Col md={12}><Form.Group><Form.Label>Reason</Form.Label><Form.Control as="textarea" rows={3} value={formData.reason} onChange={set('reason')} /></Form.Group></Col>
            </Row>
          </Modal.Body>
          <Modal.Footer className="border-0 pt-0">
            <Button variant="light" onClick={handleClose}>Cancel</Button>
            <Button variant="primary" type="submit">Save Loan</Button>
          </Modal.Footer>
        </Form>
      </Modal>
    </div>
  );
};

export default LoanList;
